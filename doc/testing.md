...
```bash
# 测试内容：
# - 创建稀疏文件（使用 dd seek= 创建空洞）
# - 验证空洞区域读取返回全零
# - 验证扩展 inode 的 sparse 字段
# - 验证数据块大小为 0 表示全零块
```
...
```bash
# 测试内容：
# - 测试块大小：4K, 16K, 64K, 128K (默认), 256K, 1M
# - 验证每种块大小都能正确挂载
# - 验证文件读取正确
# - 测试块边界读取
```
